...
`crates/slopgb-core/examples/gen_boot_logo.rs`; see **Logo art** below. The
animation's exact timing and colours were measured frame-by-frame from a legal
boot-ROM backup (the reference ROM's data and code are never copied — only its
observable output is matched). The 48-byte Nintendo logo a real cartridge carries at `$0104` is
**read from the cart** (mapped at `$0100-$01FF` during boot), never embedded
here.

## Logo art

`slopgb_logo.png` (88×24, 1-bit — 11×3 tiles) is the wordmark source. To
regenerate it:
...
(`magick` is ImageMagick 7; on ImageMagick 6 the entry point is `convert`.)
...
```sh
make                          # both ROMs (the default `all` target)
make slopgb_cgb_boot.bin      # just this one (2304 bytes, CGB-class)
```

Needs RGBDS (`rgbasm`/`rgblink`/`rgbgfx`). Load it in slopgb with
