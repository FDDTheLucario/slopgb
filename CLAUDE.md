...
- Mealybug ppu state: m3_bgp_change + m3_scx_low_3_bits pass pixel-perfect on both legs; the rest of the m3_* families need the outstanding clusters: window retrigger machine (LCDC.5 live abort/retrigger, mattcurrie's comprehensive-ppu-doc §WIN_EN), sprite fetch during prefill/discard, and the fetch-grid/per-stage sampling calibration (tile_sel/bg_map/scx_high_5_bits/scy residuals). ~31 gambatte entries are documented swaps in `baselines/gambatte.txt` pending those clusters.
