...
**PHASE B (the cc-exact reclock) — PARTIAL CONFIRM (2026-06-23 #6). The kernel/`intr_2` mutual-exclusion DISSOLVES.** Wired the deferred-commit machine advance (B1 — PPU/timer/APU/serial slaved to the deferred clock's paid debt, the S2+S3 foundation) + the dispatch retime (B2) + `early_lead`−2 (B3a), all flag-gated behind a NEW `tier2_reclock` flag (separate from `leading_edge_reads`; production + the S0 leading-edge specs byte-identical). **Result: the kernel pair SEPARATES (m2int=3 ∧ m0int=0) WHILE mooneye `intr_2_mode0_timing` PASSES — both models — so the A8 "m0int=0 forces intr_2 FAIL" exclusion is dissolved.** Pinned by gbtr `tier2_kernel_pair_matches_sameboy_target` (flag-on). Two corrections to the prior plan, both measured: **B2 (retime) is INERT** (B1+B3a do it; the −2 is absorbed by the flush), and the prescribed "dispatch 254→252" is **wrong-direction** for `int_hblank_halt`. Two residuals remain (NOT the refute condition): `intr_2_mode0_timing_sprites` (sprite-line deferred lead) + `int_hblank_halt` (the deferred halt-wake samples cc+0, SameBoy samples mid-cycle cc+2 — the B4/B5 halt-mask). Did NOT flip the defaults (full convergence + the ~7000 rebaseline remain). Detail: [`docs/hardware-state/ppu-subdot-ladder.md`](docs/hardware-state/ppu-subdot-ladder.md) "THESIS RESULT".
