...
**Debugger/viewer UI** (bgb functional-clone, branch `bgb-ui`): **F2/F3/F4** open the debugger / VRAM viewer / I/O map (or `SLOPGB_OPEN_TOOLS=debugger,vram,iomap`); the debugger also drives execution (F9 break, F7 step, F8 step-over) and has bgb-style **right-click context menus** on its panes — set/clear a breakpoint (red gutter dot; the paced loop free-runs to it via core `run_frame_until_breakpoint`), run-to-cursor, stay-on-bank pin (items not yet wired render greyed, matching the captures). Read-only introspection lives in `slopgb_core::debug` (std-only, side-effect-free `&self` — golden-safe); the frontend renders it on softbuffer — `ui::` toolkit (incl. `ui::menu` PopupMenu) + `windows::` content (per-window `WinState` for VRAM + debugger, pure `layout`/`target_at`/`on_*_click` hit-tests) + `toolwin::`/`dbg::` (breakpoint set + `DebugAction`). The VRAM viewer is interactive (tab/checkbox/radio clicks + hover details).
