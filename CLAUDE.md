...
**Half-dot grid — at its mathematically-proven ceiling (2026-06-21):** the `event_phase(kind, cc, lead_eighths)` hook + per-flip `Option<i8>` lead threading is the complete, correct event-phase model, built net-zero (S0+S1; all leads 0). Every lift lever (PalAccess per-SCX, StatMode/M0Access flip-lead, whole-dot boundary decouple) and the mandated major rewrite (a real sub-cc CPU↔PPU bus-access phase model) were attempted + instrumented; the residual class-A (~397) + class-B (~165) floor is **irreducible by a NUMERIC PROOF**, not a ceiling-by-assertion: the kernel pair `m2int_m3stat_1` (wants out3) and `m0int_m3stat_2` (wants out0) is the SAME `ldh a,(FF41)` on the byte-identical bus M-cycle (cyc=772/E=4, verified — both read `0`), so any timing model `observed=(O<E?3:0)` needs `O<4 ∧ O≥4` (unsatisfiable). The only discriminator is which ISR is on the call stack (CPU context) — outside any half-dot/sub-cc model — and tagging it breaks canonical mooneye `intr_2_mode0` ×6 + the gbmicrotest oracle. Full proof + per-lever measurements in [`docs/hardware-state/ppu-subdot-ladder.md`](docs/hardware-state/ppu-subdot-ladder.md) (PIXEL-PIPE RECLOCK — OUTCOME + IRREDUCIBILITY PROOF). The scaffold is the terminal deliverable.
