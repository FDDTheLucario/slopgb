...
pub enum Action {
    ...
    Rewind,
    /// Auto-fire the A button while held (`[`), at the Joypad "Rapid speed" rate.
    RapidA,
    /// Auto-fire the B button while held (`]`).
    RapidB,
...
pub fn map(code: KeyCode, mods: ModifiersState, focus: Focus) -> Option<Action> {
    ...
    match focus {
        ...
        Focus::Game | Focus::Viewer => match code {
            ...
            // Rapid-fire A/B (Joypad "Rapid speed") — game-side only, so `[`/`]`
            // stay the debugger's bank-step keys.
            KeyCode::BracketLeft => Some(Action::RapidA),
            KeyCode::BracketRight => Some(Action::RapidB),
            _ => None,
        },
    }
}
