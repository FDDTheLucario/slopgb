...
fn paceband_edges_are_exact() {
    ...
    assert_eq!(PaceBand::classify(target / 2 - 1, target), PaceBand::CatchUp);
    // Interior + extremes.
...
fn closed_loop_grid_settles_to_one_frame_per_tick() {
    ...
    assert_eq!(max_steady_budget, 1, "steady state must be one frame per tick");

    // Long-run production rate locked to the device clock within ±MAX_SLEW.
    let span = now.duration_since(measure_from.expect("warm-up reached")).as_secs_f64();
