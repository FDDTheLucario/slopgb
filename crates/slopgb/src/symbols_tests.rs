...
fn parse_reads_bank_addr_name_and_skips_junk() {
    ...
    assert_eq!(t.len(), 3);
    assert_eq!(t.name_at(0, 0x4000), Some("Reset"));
    assert_eq!(t.name_at(1, 0x7FFF), Some("Foo"));
    assert_eq!(t.name_at(0, 0x8000), Some("BareAddr")); // bare addr -> bank 0
    assert!(t.name_at(0, 0x1234).is_none());
}
...
fn lookups_find_exact_nearest_and_by_name() {
    ...
    assert_eq!(t.name_at(0, 0x4008), None);
    // Nearest preceding (for the memory-window status bar).
    assert_eq!(t.nearest_before(0, 0x4008), Some(("Reset", 0x4000)));
...
#[test]
fn same_address_different_bank_symbols_discriminate() {
    // Two symbols share 0x4000 in different ROM banks. Both parse (no dedup), and
    // the lookups pick the one matching the queried bank — a 0x4000 in bank 01 is
    // not named while bank 02 is mapped there (bgb behavior).
    let t = SymbolTable::parse("01:4000 Bank1Fn\n02:4000 Bank2Fn");
    assert_eq!(t.len(), 2, "both same-address symbols stored, not deduped");
    assert_eq!(t.name_at(1, 0x4000), Some("Bank1Fn"));
    assert_eq!(t.name_at(2, 0x4000), Some("Bank2Fn"));
    assert_eq!(t.name_at(0, 0x4000), None, "no bank-0 symbol at 0x4000");
    // nearest_before is likewise bank-filtered.
    assert_eq!(t.nearest_before(2, 0x4000), Some(("Bank2Fn", 0x4000)));
    assert_eq!(t.nearest_before(1, 0x4008), Some(("Bank1Fn", 0x4000)));
}
