...
fn speed_switch_pause_advances_machine_on_the_new_clock() {
    ...
    // Switching back re-paces from the read cycle on (cc + 0). The post-switch
    // CPU↔PPU realignment advances k half-dots on the leave (here k=6,
    // dsa7=4), adding k/2 = 3 machine cycles.
    b.write(0xFF4D, 0x01);
    let c0 = b.cycles();
    assert!(b.stop(0x0000, false));
