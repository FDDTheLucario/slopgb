...
pub(super) struct Sprite {
    y: u8,
    /// `pub(super)` for the sprite-at-window-X check.
    pub(super) x: u8,
    tile: u8,
    flags: u8,
    idx: u8,
...
pub(super) struct Render {
    ...
    /// (`pub(super)` with `hunt_done` for the glitch re-open, `regs.rs`.)
    ...
    // Sprites (selected during OAM scan). `pub(super)` for the
    ...
    /// `pub(super)` for the half-dot bare-exit law's clean-bare
    ...
    /// the window-length read law (`stat_irq.rs::vis_mode_read`).
    pub(super) win_aborted: bool,
    /// A window enabled at line start was disabled by a mid-line
    ...
    /// The dot a mid-mode-3 LCDC.5 RE-enable landed (0 if none this
    ...
    /// `scx & 7` at the WX-comparator match dot (the SCX effective at
    ...
    /// exit must use THIS value, not the read-time SCX
    /// (`late_scx_late_disable` fetch SCX 0 vs read SCX 4).
...
impl Render {
    ...
    pub(super) fn read_state(
        ...
        self.bg_attr = r.u8()?;
        self.bg_count = r.u8()?;
        self.phase = FetchPhase::from_u8(r.u8()?)?;
        self.fetch_x = r.u8()?;
