...
/// CGB single-speed line-start OAM-read window length (T-cycles), the dots
/// `0..N` over which SameBoy keeps OAM readable before the mode-2 lock engages
/// (`display.c:1805-1810`). See [`Ppu::cgb_linestart_oam_open`].
const CGB_LINESTART_OAM_OPEN: u16 = 4;

/// CGB double-speed line-start OAM-read window length (dots). Under double
/// speed the cc+0 read lands 2 dots earlier in the dot grid (the CPU runs at
/// 2×), so the slopgb-side window shifts down with it to keep the read-position
/// calibration: the accessible `oam_access/preread_ds*_1` read lands `dot0`
/// (open `<2`) while its render-floor `_2` sibling lands `dot2` and stays
/// blocked. SameBoy itself keeps `oam_read_blocked = false` further into DS
/// (`display.c:1789` `!cgb_double_speed` is false, so the lock only engages at
/// the unconditional `:1804`, ~dot8), reading the `_2` access accessible too —
/// but the `_2` digit is the lcd-offset RENDER shift, not the OAM read, so
/// matching SameBoy's wider DS accessibility would corrupt that render-floor
/// output; the window tracks the slopgb read grid that reproduces the
/// SameBoy-passing digits (`_1` → 0, `_2` → 3). See
/// [`Ppu::cgb_linestart_oam_open`].
const CGB_LINESTART_OAM_OPEN_DS: u16 = 2;

/// CGB single-speed palette-RAM accessible window INTO mode 3 (T-cycles), the
/// extra dots past the dot-84 mode-3 anchor over which SameBoy keeps
/// `cgb_palettes_blocked = false` before the lock engages (`display.c:1867`
/// false → `:1877` true). See [`Ppu::pal_ram_blocked`].
const PAL_M3START_OPEN: u16 = 3;

impl Ppu {
    /// A CPU VRAM write ATTEMPT landed on this line within the last 8 dots (the
    /// co-temporality discriminator, also used by the DS line-END release arm in
    /// [`Self::vram_read_blocked`]): the write's M-cycle cost is what SameBoy
    /// spreads across the following readback, so a read this close behind a write
    /// keeps the straddle-blocked view (the `vramw_m3end` want-blocked readbacks)
    /// while write-free reads resolve at their exact position. Consumed by the
    /// interconnect's VRAM-read stamp bypass (`memory.rs`).
    ...
    /// (+8 hd SS read-debt) is compared against the EMERGENT flip
    /// `2 * projected_flip_dot()` (the render's live projection — SS `dhalf` is
    /// always 0, so the whole-dot projection IS the half-dot flip) plus the 6-hd
    /// accessibility lag (SameBoy's OAM/VRAM `m0Time` trails the mode-0 flip).
    /// This resolves the `_1`/`_2` pairs whole-dot: `postread_scx3_2` (rphd 520 ≥
    ...
    /// Off-screen-window (WX=166) STALLED accessibility release: the twin of
    /// [`Self::eager_access_released`] for the off-screen window that activates
    /// during HBlank but renders NOTHING (`win_stalled`) — SameBoy's mode-3
    /// OAM/VRAM lock releases at the bare flip there (the stalled window adds no
    /// fetch), so an OAM/VRAM read past the flip is ACCESSIBLE
    /// (`m2int_wxA6_{oam,vram}busyread` [Dmg] want the readback). slopgb's render
    /// keeps `win_active` set (extending the lock to the render-done dispatch),
    /// so `eager_access_released` (`!win_active`-scoped) blocks it — this arm is
    /// the missing release, past the emergent flip (`2*projected_flip_dot + 6`,
    /// the OAM/VRAM `m0Time` lag), on WX=166 stalled sprite-free SS visible
    /// lines.
    ...
    pub(crate) fn oam_read_blocked(&self) -> bool {
        self.enabled
            ...
            // Emergent-flip release (see [`Self::eager_access_released`]).
            && !self.eager_access_released()
            // Off-screen-window (WX=166) stalled release.
            && !self.eager_offscreen_win_access()
            // CGB line-start OAM-read window: SameBoy keeps
            ...
            // `oam_read_blocked = !cgb_double_speed`). A cc+0 read landing at
            // line start (the lcd-offset-shifted `oam_access/preread_lcdoffset1_1`
            // read, slopgb `ly2 dot2` vs SameBoy `ly2 cfl0 blk0`) then sees OAM
            // accessible; slopgb locks from dot 0. Release dots `0..K` on CGB
            // (the DS window is narrower — the read grid shifts earlier; see
            // `cgb_linestart_oam_open`). Line 0 excluded (post-enable FSM has its
            // own window).
            && !self.cgb_linestart_oam_open()
            // CGB double-speed line-END OAM-read release (the render mode-3
            // LENGTH port). SameBoy releases oam/vram reads at the mode-0
            // transition (`display.c:2118`, one cycle after the
            // `!cgb_double_speed` block SKIPPED under DS), which lands the cc+0
            // read's unblock at slopgb dot `254 + SCX&7`. slopgb's plain block
            // ran to `line_render_done` (~2-3 dots later) so the `postread_ds_2`
            // read (`ly135 dot254`, SameBoy accessible) stayed blocked. See
            // [`Self::ds_lineend_open`].
            && !self.ds_lineend_open()
    }

    /// CGB double-speed line-END OAM unblock window — see
    ...
    /// drops through to `:2118`), so the cc+0 access observes OAM accessible
    /// from slopgb dot `254 + SCX&7`. slopgb's plain block ran to
    /// `line_render_done` (~2 dots later), so the `oam_access/postread_ds_2`
    /// read (`ly135 dot254`, SameBoy accessible) stayed blocked while its `_1`
    /// sibling (dot252, blocked) passed. Extends the SAME boundary to OAM WRITES
    /// (`postwrite_ds_2` write@254 lands / `_1` @252 dropped; `postwrite_scx1_ds`
    /// 256/254 — the write release rides the identical `254 + SCX&7` law). Bare
    ...
    /// CGB line-start OAM-read window — see [`Self::oam_read_blocked`]. SameBoy
    /// carries `oam_read_blocked = false` from the previous HBlank across the
    /// first few dots of a visible line until the mode-2 lock engages
    /// ([`CGB_LINESTART_OAM_OPEN`] single speed / [`CGB_LINESTART_OAM_OPEN_DS`]
    /// double speed).
    fn cgb_linestart_oam_open(&self) -> bool {
        ...
        self.model.is_cgb() && self.line != 0 && in_window
    }
    ...
    pub(crate) fn oam_write_blocked(&self) -> bool {
        ...
        if self.model.is_cgb() {
            ...
            return if self.dot < 4 {
                ...
                // The mode3→0 write-unblock coincides with the visible mode→0
                // flip (`vis_early`), one dot before `line_render_done` — the
                // same coupling as the read side (see `oam_read_blocked`); the
                // DS line-END release rides the same `254 + SCX&7` law as reads
                // ([`Self::ds_lineend_open`]).
                self.dot < 84
                    || (!self.line_render_done
    ...
    /// The mode3→0 write-unblock coincides with the visible mode→0 flip
    ...
    pub(crate) fn vram_read_blocked(&self) -> bool {
        if !self.enabled
            ...
            // Off-screen-window (WX=166) stalled release.
            || self.eager_offscreen_win_access()
        // NOTE: the DS line-END read release is NOT applied to VRAM.
        // `vram_m3/postread_ds_2` (want accessible @dot254) is CO-TEMPORAL with
        // `vramw_m3end/vramw_m3end_ds_2` (want the readback BLOCKED @dot254): the
        // vramw write costs a CPU M-cycle that shifts SameBoy's readback cfl vs
        // the sprite-free postread, but slopgb's frame collapses both to the
        // same dot254 read — so a VRAM release is an A/B swap (+1 postread / −1
        // vramw). OAM has no write-end readback at that dot, so its release
        // ([`Self::ds_lineend_open`], wired only into `oam_read_blocked`) is
        // clean. The VRAM DS line-END release stays parked.
        {
            return false;
        }
        ...
        // `preread_lcdoffset1_1` law-dot82 opens where the raw dot83 blocked,
        // `_2` law-dot86 stays blocked — a clean separation.
        let d = self.law_pos().1;
        // The DS VRAM read frame: (a) the cc+0 read's true DS sample sits +3 T
        // late, so the mode-3 entry lock covers it from dot 80 (the SS +3 CGB
        // grace does not apply on the DS grid: `preread_ds_2` reads dot82 wanting
        // BLOCKED); (b) the line-END release mirrors the OAM release at
        // 254 + SCX&7, EXCEPT within 8 dots of a same-line CPU VRAM write attempt
        // (the
    ...
    pub(super) fn vram_write_blocked(&self) -> bool {
        ...
        if self.glitch_line {
            ...
            // ~dot82 write BLOCKED while `_1`'s earlier write lands).
            ...
            // ends on the visible mode→0 flip (`write_unblocked_early`).
    ...
    pub(super) fn pal_ram_blocked(&self) -> bool {
        ...
        if self.render_finished {
            // The palette-RAM unblock trails the pipe end by 1 dot at single
            // speed / 0 in double speed on the cc+0 frame. The full
            // `cgbpal_m3end` constraint table: SS reads at `pipe_end` are
            // blocked (`m3end_1` @256, `scx2_1` @256, `scx5_1` @260) while reads
            // at/past `pipe_end + 1` are open (`_2` @260 with pipe 256/258/259,
            // `scx5_2` @264 with 261 — Δ = +2 is REFUTED by `scx3_2` read@260
            // pipe 259); DS reads open AT the pipe end (`m3end_ds_2` @256 pipe
            // 256, `scx5_ds_2` @262 pipe 261) while pre-pipe reads stay blocked
            // (`ds_1` @254, `scx5_ds_1` @260 < 261).
            return self.model.is_cgb()
                && !self.glitch_line
                && self.pal_open_dot != 0
        ...
        let lock = if self.glitch_line {
            ...
            // BLOCKED (read FF / write dropped) while the `_1` legs at dot 80
            // land — so the base boundary is 84,
