...
//! IE, the STAT sub-dot edges, the OAM-DMA + HDMA engines, CGB misc regs).
//!
//! NOT serialized: `model`/`cgb_mode` (ROM-derived — a state loads into a
//! same-model machine) and the debugger-only fields (watchpoints, profiler,
//! exception mask) — those are live UI state, left untouched by a load.
