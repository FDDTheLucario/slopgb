...
| Plugins | Per-plugin **enable** checkbox (`Field::PluginEnable(i)` → `PluginConfig.entries[i].enabled` → `PluginHost::set_enabled`), for tier-1 and tier-3 plugins alike: a disabled tier-1 plugin's `on_frame` stops at the next pump, a disabled tier-3 SUBSYSTEM plugin (`spc700`/`w65c816`/`snes-ppu`/`msu1`) leaves its slot empty from the next reset / ROM load — the list carries a note saying so. Plus the read-only plugins-dir display and an **allow-mutation** toggle (`Field::PluginAllowMutation`, default off). No bgb equivalent — see [plugin-api.md](plugin-api.md#managing-plugins-from-the-ui). |
