...
- STAT/VBlank rises inside the LCD squash window `ack_squash_dots`: **2** dots at single speed (the two T-cycles left of the low push's M-cycle), **6** when the ack lands on line 0 or 153 or on a line's first dot — the line-start LYC/OAM emissions there sit one M-cycle past our ack where SameBoy's clear already covers them (`ly0/lycint152_lyc{0,153}irq_late_retrigger`, `lyc153int_m2irq_late_retrigger`, `m1/lycint{143_m1irq,_vblankirq}_late_retrigger`) — **1** in double speed and **2** there when HBlank is the enabled STAT source (`Ppu::stat_src_hblank`, the `late_m0irq_retrigger` family).

  The 6-dot widening is **CGB-except-line-0** (2026-08-06, +4/−0: the four rows above's [Cgb] legs). CGB decouples its own line-0 emissions to dot 4 — a full M-cycle past a dot-0 ack, so the dispatch cannot fold them in and the rise is DELIVERED. The discriminator is the ack line, measured per row: the four lifted rows ack on line 153 or 144 (`lycint152_lyc0irq_2` at 153:8, `lycint152_lyc153irq_2` at 153:0, `lycint143_m1irq_2` and `lycint_vblankirq_2` at 144:0) where the emission is co-instant; `lyc153int_m2irq_late_retrigger_1` [Cgb] acks at 0:0 and wants delivery, and an unscoped CGB widening is exactly what costs it (+4/−1). Gating on the source instead (`stat_rise_oam`) does NOT separate them — measured, both read false at the ack. DMG keeps line 0: its pulse is not decoupled. Unit test `cgb_line0_acknowledge_takes_no_line_start_widening`.
